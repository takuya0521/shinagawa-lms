...
@section('page-style', 'resources/css/pages/teacher/attendance/edit.css')
...
@section('content')
    <section class="space-y-6">
        <div class="rounded-2xl bg-white p-6 shadow-sm">
            <h1 class="text-2xl font-bold text-slate-900">出欠登録</h1>
            <p class="mt-2 text-sm text-slate-600">担当授業と授業日を選択し、対象生徒全員の出欠を登録します。</p>
        </div>


        <div class="rounded-2xl bg-white p-6 shadow-sm">
            <form method="GET" action="{{ route('teacher.attendance.edit') }}" class="grid gap-4 md:grid-cols-[minmax(0,1fr)_220px_auto]">
                <div>
                    <label for="timetable_slot_id" class="block text-sm font-semibold text-slate-700">担当授業・時間割枠</label>
                ...
                <div class="flex items-end">
                    <button type="submit" class="w-full rounded-lg bg-slate-900 px-5 py-2.5 font-semibold text-white hover:bg-slate-700">対象生徒を表示</button>
                </div>
            </form>
        </div>
        ...
        @if ($lessonSession !== null && $selectedSlot !== null)
            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm">
                <div class="border-b border-slate-200 pb-5">
                    ...
                    <h2 class="mt-1 text-xl font-bold text-slate-900">{{ $selectedSlot->course->course_name }}</h2>
                ...
                @else
                    <form method="POST" action="{{ route('teacher.attendance.update', $lessonSession) }}" class="mt-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-slate-200">
                                <thead class="bg-slate-50">
                                    <tr>
                                        <th class="px-5 py-3 text-left text-xs font-semibold text-slate-500">生徒番号</th>
                                        <th class="px-5 py-3 text-left text-xs font-semibold text-slate-500">氏名</th>
                                    ...
                                    @foreach ($students as $student)
                                        ...
                                        <tr>
                                            <td class="whitespace-nowrap px-5 py-4 text-sm font-semibold text-slate-900">
                                                {{ $student->student_no ?? '-' }}
                                                <input type="hidden" name="records[{{ $studentIndex }}][student_id]" value="{{ $student->id }}">
                                            </td>
                                            <td class="px-5 py-4 text-sm text-slate-700">{{ $student->student_name }}</td>
                                            <td class="px-5 py-4">
                                                <select name="records[{{ $studentIndex }}][attendance_status]" required class="rounded-lg border border-slate-300 px-3 py-2 text-sm">
                                                    @foreach ($attendanceStatuses as $attendanceStatus)
                                                        <option value="{{ $attendanceStatus->value }}" @selected(old("records.{$studentIndex}.attendance_status", $attendanceRecord?->attendance_status->value ?? \App\Enums\AttendanceStatus::Present->value) === $attendanceStatus->value)>
                                            ...
                                            <td class="px-5 py-4">
                                                <input name="records[{{ $studentIndex }}][note]" maxlength="255" value="{{ old("records.{$studentIndex}.note", $attendanceRecord?->note) }}" class="block min-w-64 rounded-lg border border-slate-300 px-3 py-2 text-sm" placeholder="任意">
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="mt-6 flex justify-end gap-3 border-t border-slate-200 pt-6">
                            <a href="{{ route('teacher.attendance.index') }}" class="rounded-lg border border-slate-300 px-5 py-3 font-semibold text-slate-700 hover:bg-slate-50">戻る</a>
                            <button type="submit" class="rounded-lg bg-slate-900 px-5 py-3 font-semibold text-white hover:bg-slate-700">{{ $students->count() }}件を保存</button>
                        </div>
                    </form>
                @endif
            </div>
