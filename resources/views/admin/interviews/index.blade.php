...
@section('page-style', 'resources/css/pages/admin/interviews/index.css')
...
@section('content')
    <section class="space-y-6">
        <div class="flex flex-col gap-4 rounded-2xl bg-white p-6 shadow-sm sm:flex-row sm:items-start sm:justify-between">
            <div>
                <p class="text-sm font-semibold text-slate-500">A-031</p>
                <h1 class="mt-1 text-2xl font-bold text-slate-900">面談記録一覧</h1>
                <p class="mt-2 text-sm text-slate-600">
                    全生徒の面談履歴、担当教員、次回対応、関連リンクを確認します。
                </p>
            </div>
            ...
            <form method="GET" action="{{ route('admin.interviews.index') }}" class="space-y-4">
                <div class="grid gap-4 lg:grid-cols-4">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <input id="keyword" name="keyword" value="{{ $keyword }}" placeholder="生徒番号・氏名・担当教員・面談メモ・次回対応" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2">
                    </div>
                </div>
                ...
                <div class="grid gap-4 lg:grid-cols-4">
                    ...
                    <div>
                        <label for="teacher_id" class="block text-sm font-semibold text-slate-700">担当教員</label>
                        <select id="teacher_id" name="teacher_id" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2">
                            <option value="">すべて</option>
                    ...
                    <div class="flex items-end gap-3">
                        ...
                        <a href="{{ route('admin.interviews.index') }}" class="rounded-lg border border-slate-300 px-5 py-2.5 font-semibold hover:bg-slate-50">クリア</a>
                    </div>
                </div>
            </form>
        </div>
                            ...
                            <tr class="align-top hover:bg-slate-50">
                                ...
                                <td class="px-5 py-4 text-sm">
                                    ...
                                    <p class="mt-1 text-xs text-slate-500">
                                        {{ $interviewRecord->student->student_no ?? '番号未設定' }} / {{ $interviewRecord->student->grade->label() }} / {{ $interviewRecord->student->classGroup->class_name }}
                                    </p>
                                </td>
                                ...
                                <td class="max-w-sm px-5 py-4 text-sm text-slate-700">
                                    {{ $interviewRecord->next_action !== null ? \Illuminate\Support\Str::limit($interviewRecord->next_action, 100) : '-' }}
                                </td>
                                <td class="px-5 py-4 text-sm">
                                ...
                                <td class="whitespace-nowrap px-5 py-4 text-right text-sm">
                                    <a href="{{ route('admin.interviews.edit', $interviewRecord) }}" class="font-semibold text-blue-700 hover:text-blue-900">詳細・編集</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-sm text-slate-500">条件に一致する面談記録はありません。</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
