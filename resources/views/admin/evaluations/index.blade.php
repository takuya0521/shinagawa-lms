    ...
    <div class="space-y-6">
        ...
        <section class="p-6 lms-panel">
            <form method="GET" action="{{ route('admin.evaluations.index') }}" class="grid gap-4 lg:grid-cols-4">
                ...
                <div>
                    <label for="student_id" class="block text-sm font-semibold lms-text-neutral-secondary">生徒</label>
                    <select
                        id="student_id"
                        name="student_id"
                        class="mt-2 w-full rounded-lg border px-3 py-2 lms-form-control"
                ...
                <div class="flex items-end">
                    <label
                        class="flex w-full items-center gap-2 rounded-lg border lms-border-neutral-default px-3 py-2.5
                            text-sm font-semibold lms-text-neutral-secondary"
                    >
                        ...
                        @forelse ($evaluations as $evaluation)
                            <tr>
                                <td class="px-5 py-4 text-sm">
                                    <p class="font-semibold">{{ $evaluation->student_name }}</p>
                                    <p class="mt-1 text-xs lms-text-neutral-muted">
                                        {{ $evaluation->student_no ?? '番号未設定' }} / {{ $evaluation->class_name }}
                                    </p>
                                </td>
                                <td class="px-5 py-4 text-sm">
                                    <p class="font-semibold">{{ $evaluation->course_name }}</p>
                                    <p class="mt-1 text-xs lms-text-neutral-muted">
                                        {{ $evaluation->subject_name }} /
                                        担当 {{ $evaluation->teacher_name ?? '未設定' }}
                                    </p>
                                </td>
                                @if ($evaluation->evaluation_id !== null)
                                    <td
                                        class="whitespace-nowrap px-5 py-4 text-right text-sm"
                                    >{{ number_format((float) $evaluation->submission_score, 2) }} / {{
                                    number_format((float) $evaluation->attendance_score, 2) }} / {{
                                    ...
                                    >
                                    <a
                                        href="{{ route('admin.evaluations.edit', $evaluation->evaluation_id) }}"
                                ...
                                @else
                                    ...
                                    <td class="whitespace-nowrap px-5 py-4 text-right text-sm">-</td>
                                @endif
                            </tr>
                        @empty
                            <tr>
